...
public class GifDecoder {
    ...
    protected int loopCount = 100; // iterations; 0 = repeat forever
    ...
    public synchronized int getDelay(int n) {
        delay = -1;
        if ((n >= 0) && (n < frameCount) && (n < frames.size())) {
    ...
    public synchronized Bitmap getFrame(int n) {
        if (frameCount <= 0)
            return null;
        n = n % frameCount;
        if (n >= frames.size()) return null;
